...
#define LOG_PS2_INT(...)
	...
	void ps2_gpio_read_interrupt_handler()
	{
		...
		if (data->cur_read_pos == PS2_GPIO_POS_START) {
			...
			if (sda_val != 0) {
				LOG_PS2_INT("Ignoring read interrupt due to invalid start bit.",
					    NULL);
...
#define PS2_GPIO_E_WRITE_FAILURE 5
	...
	int ps2_gpio_write_byte(uint8_t byte)
	{
		...
		for (int i = 0; i < PS2_GPIO_WRITE_MAX_RETRY; i++) {
			...
			if (err == 0) {
				if (i > 0) {
					LOG_WRN("Successfully wrote 0x%x on try #%d of %d...", byte,
	...
	int ps2_gpio_write_byte_blocking(uint8_t byte)
	{
		...
		if (data->cur_write_status == PS2_GPIO_WRITE_STATUS_SUCCESS) {
			...
			LOG_ERR("Blocking write finished with failure for byte 0x%x status: %d",
...
#if IS_ENABLED(CONFIG_PS2_GPIO_INTERRUPT_LOG_ENABLED)
		k_work_init_delayable(&interrupt_log_scl_timout,
				      ps2_gpio_interrupt_log_scl_timeout);
	...
	DEVICE_DT_INST_DEFINE(0, &ps2_gpio_init, NULL, &ps2_gpio_data, &ps2_gpio_config,
			      POST_KERNEL, CONFIG_PS2_INIT_PRIORITY, &ps2_gpio_driver_api);
